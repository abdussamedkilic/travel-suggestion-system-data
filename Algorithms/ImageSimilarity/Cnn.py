...
class Cnn:
    ...
    def get_img_size_model(self, model):
        ...
        if model_name == 'vgg16':
            ...
            img_size_model = (224, 224)
        print("Warning: model name unknown. Default image size: {}".format(
            img_size_model))
    ...
    def get_layername_feature_extraction(self, model):
        ...
        print("Warning: model name unknown. Default layername: '{}'".format(
            layername_feature_extraction))
    ...
    def get_feature_vector(self, model, img_path):
        ...
        layername_feature_extraction = self.get_layername_feature_extraction(
            model)
        model_feature_vect = Model(inputs=model.input, outputs=model.get_layer(
            layername_feature_extraction).output)
