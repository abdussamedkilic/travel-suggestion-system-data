...
class Bert():
    ...
    def sentential_embeddings(self, tokenizer, tokenized_text):
        ...
        self.idx_tokens = self.tokenizer.convert_tokens_to_ids(
            self.tokenized_text)
        ...
        with torch.no_grad():
            self.outputs = self.model(
                self.tokens_tensor, self.segmenter_tensor)
    ...
    def calculate_distance(self, sentence_1, sentence_2):
        ...
        self.sentence_1, self.vs_sum_cat1 = self.sentential_embeddings(
            self.tokenizer, self.tokenized_text_1)
        self.sentence_2, self.vs_sum_cat2 = self.sentential_embeddings(
            self.tokenizer, self.tokenized_text_2)
